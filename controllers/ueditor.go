...
func (c *UeditorController) ControllerUE() {
	...
	case "uploadimage", "uploadfile", "uploadvideo":
		...
		err := os.MkdirAll(".\\static\\attachment\\", 0777) //..代表本当前exe文件目录的上级，.表示当前目录，没有.表示盘的根目录
		if err != nil {
			logs.Error(err)
		}
		//保存上传的图片
		...
		newfilename := time.Now().Format("20060102150405") + fileext  //6-1-2 3:4:5

		path1 := ".\\static\\attachment\\" + newfilename
		err = c.SaveToFile("upfile", path1) //.Join("attachment", attachment)) //存文件    WaterMark(path)    //给文件加水印
		if err != nil {
			logs.Error(err)
		}
		c.Data["json"] = map[string]interface{}{"state": "SUCCESS", "url": "/static/attachment/" + newfilename , "title": newfilename , "original": newfilename}
		c.ServeJSON()
		// 		{
		//     "state": "SUCCESS",
		//     "url": "upload/demo.jpg",
	...
	case "uploadscrawl":
		...
		name := c.Input().Get("name")
		err := os.MkdirAll(".\\static\\attachment\\"+number+name, 0777) //..代表本当前exe文件目录的上级，.表示当前目录，没有.表示盘的根目录
		if err != nil {
			logs.Error(err)
		}
		path1 := ".\\static\\attachment\\" + number + name + "\\"
		//保存上传的图片
		...
		c.Data["json"] = map[string]interface{}{
			"state":    "SUCCESS",
			"url":      "/static/attachment/" + newname + ".jpg",
			"title":    newname + ".jpg",
